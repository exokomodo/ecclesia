...
(defun run-tests ()
  ...
  (let ((img (assemble *bootloader*)))
    ...
    ;; ── Code starts with CLI (0xFA) ──────────────────────────────────────
    (assert= #xfa (aref img 0)
             "First byte is CLI (0xFA)")

    ;; ── Banner null terminator present ───────────────────────────────────
    (let* ((banner-start +code-size+)
           (null-pos (loop for i from banner-start below 512
                           when (= (aref img i) 0) return i)))
      (assert= t (not (null null-pos))
               "Banner null terminator found in image")
      (assert= (+ banner-start (1- (length (banner-db-forms))))
               null-pos
               "Banner null terminator at expected offset")))
